...
import { getUnderClearanceData, syncUnderClearanceData } from "@/app/api/client/reports/under-clearance/UnderClearanceApiClient";
...
export default function UnderClearanceClientReport() {
  ...
  // Helper function to load "Under Clearance" data specifically
  const loadUnderClearanceData = useCallback(async() => {
    ...
    const params: {
      ...
      page: 1,
      limit: 0,
      jobStatusType: 'Under Clearance', // Filter specifically for "Under Clearance" status
  ...
  }, [loadUnderClearanceData, loading]);

  const syncAndUpdateData = useCallback(async() => {
    setIsSyncing(true);
    try {
      const result = await syncUnderClearanceData();

      if (!result.success) {
        throw new Error('Failed to sync Under Clearance data');
      }
      refresh();
      notify('Under Clearance data synced successfully', 'success', 3000);
    } catch (error) {
      console.error('Error syncing Under Clearance data:', error);
    ...
    <div className='view crm-contact-list'>
      <div className='view-wrapper view-wrapper-contact-list list-page'>
        ...
        >
          ...
          <SearchPanel visible placeholder='Search Under Clearance Jobs' />
